...
from invoicescanner.malik_excel import *
...
def upload(request):
    ...
    if request.method=='POST':
        ...
        if name.endswith('.xlsx'):
            ...
            text_file.write("malik found (Excel sheet)")
            file_path = os.path.join(os.getcwd(), 'media', name)
            print("Excell sheet found "*10,file_path)
        else:
...
def fields(request):
    ...
    if 'malik found (Excel sheet)' in open(file_location).read():
        obj = Data_parser(file_path)
        data = obj.field_data_malik()
        print("malik found " * 3, data)
